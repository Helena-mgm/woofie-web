...
const appNavItems: NavItem[] = [
  ...
  { name: 'Chiens perdus', href: '/lost-dogs', icon: '🚨' },
...
export function Header() {
  ...
  const profileRole = authenticatedUser
    ? authenticatedUser.type === 'owner' ? 'Propriétaire' : 'Dog-sitter'
    : '';
    ...
    <header className="bg-white shadow-sm sticky top-0 z-50">
          ...
          <div className="flex items-center justify-between gap-4">
              ...
              <nav className="hidden flex-1 items-center justify-center gap-2 min-[1200px]:flex xl:gap-3">
                {appNavItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`relative inline-flex items-center gap-1.5 rounded-2xl border px-3 py-2 text-sm font-semibold transition-all duration-150 hover:-translate-y-0.5 active:scale-[0.97] xl:gap-2 xl:px-4 ${
                          isActive
              ...
              {!isAuthenticated ? (
                ...
                <div className="relative" ref={menuRef}>
                  ...
                  >
                    ...
                    <svg
                      className={`h-4 w-4 shrink-0 text-gray-600 transition-transform duration-200 ${isMenuOpen ? 'rotate-180' : ''}`}
                  ...
                  >
                    <div className="py-1">
                      <Link href={profileHref} onClick={() => setIsMenuOpen(false)} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                        <svg className="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                        <span className="font-medium text-gray-700">Mon profil</span>
                      </Link>
                      <Link href="/dashboard/settings" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                        <svg className="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex items-center gap-2 min-[1200px]:hidden">
              ...
              {isAuthenticated ? (
                ...
                >
                  <span className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-[#D2691E] to-[#8B4513] text-white font-semibold text-sm relative">
                    {authenticatedUser?.photo_path ? (
                      <Image src={getImageUrl(authenticatedUser.photo_path)} alt={avatarAlt} fill className="object-cover" sizes="32px" />
                    ) : (
                      profileInitial
                    )}
                  </span>
                ...
                <div className="flex items-center gap-2">
                  <Link href="/login" className="rounded-full border border-[#F1E5D4] bg-white/90 px-3 py-1.5 text-sm font-semibold text-[#8B4513] shadow-sm transition hover:bg-[#FFF2E0]">
                    Connexion
                  </Link>
                  <Link href="/register" className="rounded-full bg-[#8B4513] px-3 py-1.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#A0522D]">
                    Inscription
                  </Link>
                </div>
              )}
      ...
      <div
        ...
        className={`fixed inset-0 z-40 bg-black transition-opacity duration-200 ${
          isMobileNavOpen ? 'opacity-45 pointer-events-auto' : 'opacity-0 pointer-events-none'
        }`}
      />
        ...
        <div className="overflow-y-auto px-5 pt-6 pb-4">
          ...
          {isAuthenticated && (
            <div className="mt-5 rounded-2xl border border-[#F1E5D4] bg-[#FFF9F1] p-4">
              <Link href={profileHref} onClick={() => setIsMobileNavOpen(false)} className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-[#D2691E] to-[#8B4513] text-lg font-semibold text-white relative">
                  {authenticatedUser?.photo_path ? (
                    <Image src={getImageUrl(authenticatedUser.photo_path)} alt={avatarAlt} fill className="object-cover" sizes="48px" />
                  ) : (
                    profileInitial
                  )}
                </div>
        ...
        <div className="border-t border-[#F6E5D4] bg-[#FFF8EF] px-5 py-4">
          {isAuthenticated ? (
            <div className="flex flex-col gap-3">
              <Link href="/dashboard/settings" onClick={() => setIsMobileNavOpen(false)}
            ...
            <div className="flex flex-col gap-3">
              <Link href="/login" onClick={() => setIsMobileNavOpen(false)} className="flex items-center justify-center rounded-full border border-[#F6E5D4] px-4 py-2 text-sm font-semibold text-[#8B4513] transition hover:bg-white">
                Connexion
              </Link>
              <Link href="/register" onClick={() => setIsMobileNavOpen(false)} className="flex items-center justify-center rounded-full bg-[#8B4513] px-4 py-2 text-sm font-semibold text-white shadow-md transition hover:bg-[#A0522D]">
                Inscription
              </Link>
            </div>
          )}
